...
const readCatalog = unstable_cache(
  async (slug: string | null, category: string | null) => {
    await initDb();
  ...
  ["public-catalog"],
  { tags: ["public-catalog"], revalidate: 60 },
);

export function getCatalogProducts(filter: CatalogFilter = {}) {
  return readCatalog(filter.slug || null, filter.category || null);
}
